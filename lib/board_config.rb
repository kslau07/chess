...
class BoardConfig
  ...
  def standard
    ...
    (0..7).each { |sq| board.grid[1][sq] = pieces[:white_pcs][sq] } # wht front row
    (0..7).each { |sq| board.grid[6][sq] = pieces[:black_pcs][sq] } # blk front row
    (0..7).each { |sq| board.grid[0][sq] = pieces[:white_pcs][sq+8] } # wht back row
    (0..7).each { |sq| board.grid[7][sq] = pieces[:black_pcs][sq+8] } # blk back row
  end
  ...
  def checkmate_scenarios
    ...
    # mv_list = ["Pd2d4", "Pa7a6", "Pd4d5", "Pe7e5"]
    # move_list.instance_variable_set(:@all_moves, mv_list)
  end
  ...
  def en_passant_white_version1
    ...
    # mv_list =  ["Pd2d4+", "Pc7c6+", "Pd4d5+", "Pc6c5+"] # invalid, blk pawn moved twice in list
    # board.grid[4][3] = PieceFactory.create('Pawn', 'white')
